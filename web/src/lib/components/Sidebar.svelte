<script lang="ts">
	...
	onMount(async () => {
		initTheme();
		const fetched = await api.get<{ id: string; name: string; photo_url?: string | null; wallpaper_url?: string | null }[]>('/api/v1/households');
	...
	const iconStyle = $derived(collapsed ? 'max-width:0;opacity:0;overflow:hidden' : 'max-width:20px;opacity:1');
</script>
...
<div class="flex flex-col h-full select-none overflow-hidden">
	...
	<div class="py-4 px-3 shrink-0 flex items-center">
		...
		>
			...
			<span class="whitespace-nowrap transition-[max-width,opacity] duration-200" style={textStyle}>
				Homeboard
			</span>
		</a>
	</div>
		...
		<Popover.Root bind:open={switcherOpen}>
			...
			<Popover.Content class="w-56 p-1 gap-0" align="start" side={collapsed ? 'right' : 'bottom'}>
				{#each $households as family (family.id)}
					...
					>
						<HouseholdAvatar name={family.name} photoUrl={family.photo_url} size={20} />
						<span class="flex-1 truncate">{family.name}</span>
