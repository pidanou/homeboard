...
func (r *EventRepository) Update(ctx context.Context, event *model.Event) error {
	_, err := r.pool.Exec(ctx,
		`UPDATE events SET title=$1, description=$2, location=$3, start_at=$4, end_at=$5,
		  all_day=$6, category_id=$7, recurrence_rule=$8, updated_at=$9, birthday_of=$10, important=$11
		 WHERE id=$12 AND family_id=$13`,
		event.Title, event.Description, event.Location, event.StartAt, event.EndAt,
		event.AllDay, event.CategoryID, event.RecurrenceRule, event.UpdatedAt, event.BirthdayOf, event.Important,
...
func scanEvent(row scannable, e *model.Event) error {
	return row.Scan(
		...
		&e.CreatedBy, &e.CreatedAt, &e.UpdatedAt, &e.Type, &e.BirthdayOf, &e.Important,
		&e.AttendeeIDs,
	)
}
