...
func (r *EventRepository) Create(ctx context.Context, event *model.Event) error {
	_, err := r.pool.Exec(ctx,
		`INSERT INTO events (id, family_id, title, description, location, start_at, end_at, all_day,
		  category_id, recurrence_rule, recurrence_parent_id, recurrence_date, cancelled, created_by, created_at, updated_at, type, icon, birthday_of, important)
		...
		event.CreatedBy, event.CreatedAt, event.UpdatedAt, event.Type, event.Icon, event.BirthdayOf, event.Important,
	)
	if err != nil {
		return err
	}
...
func (r *EventRepository) ListByFamilyAndRange(ctx context.Context, familyID string, from, to time.Time) ([]*model.Event, error) {
	...
	rows, err := r.pool.Query(ctx,
		`SELECT e.id, e.family_id, e.title, COALESCE(e.description,''), COALESCE(e.location,''),
		        ...
		        e.created_by, e.created_at, e.updated_at, e.type, e.icon, e.birthday_of, e.important,
...
func (r *EventRepository) Update(ctx context.Context, event *model.Event) error {
	_, err := r.pool.Exec(ctx,
		`UPDATE events SET title=$1, description=$2, location=$3, start_at=$4, end_at=$5,
		  all_day=$6, category_id=$7, recurrence_rule=$8, icon=$9, updated_at=$10, birthday_of=$11, important=$12
		 WHERE id=$13 AND family_id=$14`,
		event.Title, event.Description, event.Location, event.StartAt, event.EndAt,
		event.AllDay, event.CategoryID, event.RecurrenceRule, event.Icon, event.UpdatedAt, event.BirthdayOf, event.Important,
...
func scanEvent(row scannable, e *model.Event) error {
	return row.Scan(
		...
		&e.CreatedBy, &e.CreatedAt, &e.UpdatedAt, &e.Type, &e.Icon, &e.BirthdayOf, &e.Important,
		&e.AttendeeIDs,
	)
}
